        ...
        if "questions" in data:
            ...
            def sort_key(q_key):
                if q_key.startswith('easy_'):
                    ...
                    # 兼容其他格式
                    return (3, int(q_key.replace('question', '')))
...
class MinimumCutGenerator:
    ...
    def generate_single_question(self) -> Tuple[Dict, List[List[int]], int]:
        ...
        for _ in range(max_attempts):
            if self.ensure_path_exists:
                graph = self.generate_connected_graph(num_nodes)
            else:
    ...
    def set_difficulty_params(self, difficulty: str):
        """根据难度设置参数"""
        if difficulty == "easy":
            # 如需保证"真最优"，可把上限调到 <= exact_threshold
            self.num_nodes_range = (15, 20)
            self.edge_density = 0.8
            self.min_cut_size = 10
        elif difficulty == "medium":
            self.num_nodes_range = (20, 30)
            self.edge_density = 0.85
            self.min_cut_size = 15
        elif difficulty == "hard":
            self.num_nodes_range = (30, 40)
            self.edge_density = 0.9
            self.min_cut_size = 20
    ...
    def generate_questions(self, num_questions: int, difficulty: str = "medium") -> Dict:
        ...
        # 确定big_small标记
        avg_nodes = sum(len(q) for q in questions.values()) / len(questions)
        big_small = 'small'
    ...
    def generate_mixed_difficulty_questions(self, easy_count: int = 0, medium_count: int = 0, hard_count: int = 0) -> Dict:
        """
        生成混合难度的问题集（平衡最小割），顺序为easy->medium->hard
        """
        all_questions = {}
        all_answers = {}
        all_metadata = {}
...
示例用法:
  ...
  # 生成分离文件
  python generate_questions.py --easy 5 --medium 5 --hard 5 --separate
  ...
  python generate_questions.py --num_questions 5 --difficulty medium
    ...
    parser.add_argument('--difficulty', choices=['easy', 'medium', 'hard'], default='medium', help='难度级别 (传统模式)')
    ...
    if args.num_questions is not None:
        ...
        if total_questions == 0:
            ...
            print("使用 --easy N, --medium M, 或 --hard P 来指定各难度题目数量")
