#!/usr/bin/env python3
"""
基于规则的最大独立集问题（Maximum Independent Set）造题脚本
"""
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for i, q_key in enumerate(sorted_q_keys):
            ...
            for j, node_key in enumerate(sorted_node_keys):
                ...
                adj_str = ", ".join([f'"{k}": {v}' for k, v in sorted(adj_dict.items(), key=lambda item: int(item[0]))])
...
class MaxIndependentSetGenerator:
    """最大独立集问题生成器"""
    ...
    def generate_single_question(self) -> Tuple[Dict, List[int]]:
        """
        生成单个最大独立集问题，并确保有解。
        方法：先确定一个独立集，然后围绕它构建图。
        """
        num_vertices = random.randint(*self.p["num_vertices"])
        solution_size = random.randint(*self.p["solution_size"])
        solution_size = min(solution_size, num_vertices)

        # 1. 将顶点分为独立集（I）和其他顶点（O）
        ...
        # 3. 在独立集（I）和其他顶点（O）之间添加边
        for o_node in other_vertices:
            # 确保每个其他顶点至少与独立集中的一个顶点相连
            ...
            for i_node in independent_set:
                if i_node != i_node_to_connect and random.random() < self.p["edge_density_IO"]:
                    adj[str(o_node)][str(i_node)] = 1
                    adj[str(i_node)][str(o_node)] = 1

        # 4. 确认独立集（I）内部没有边（这一步由算法保证，无需代码）
        
        return adj, independent_set

    def get_difficulty_params(self, difficulty: str) -> Dict:
        """获取指定难度的参数"""
        if difficulty == "easy":
            return {"num_vertices": (12, 20), "solution_size": (4, 8), 
                    "edge_density_O": 0.4, "edge_density_IO": 0.4}
        elif difficulty == "medium":
            return {"num_vertices": (20, 30), "solution_size": (8, 12),
                    "edge_density_O": 0.4, "edge_density_IO": 0.4} 
        elif difficulty == "hard":
            return {"num_vertices": (30, 40), "solution_size": (12, 16),
                    "edge_density_O": 0.4, "edge_density_IO": 0.4} 
        elif difficulty == "bench":
            return {"num_vertices": (40, 50), "solution_size": (16, 20),
                    "edge_density_O": 0.4, "edge_density_IO": 0.4} 
        else:
...
def main():
    parser = argparse.ArgumentParser(description='生成最大独立集问题数据集')
    ...
    parser.add_argument('--output', type=str, default='generated_maximum_set_questions.json', help='输出文件名')
    ...
    generator = MaxIndependentSetGenerator({})
