...
class SubsetSumSolver:
    ...
    def _solve_dp(self):
        ...
        dp[0][0] = 0  # Base case: sum of 0 is possible with 0 items.
        ...
        for i in range(1, self.num_items + 1):
            ...
            for j in range(self.target + 1):
                ...
                # The cardinality is the same as for the previous i-1 items.
                dp[i][j] = dp[i - 1][j]

                # Option 2: Include the current number.
                if j >= num_value and dp[i - 1][j - num_value] != -1:
                    # If including the number gives a higher cardinality, take it.
                    dp[i][j] = max(dp[i][j], dp[i - 1][j - num_value] + 1)
        
        # --- Backtrack to find which numbers were included ---
        if dp[self.num_items][self.target] == -1:
            return "No solution", 0
        ...
        for i in range(self.num_items, 0, -1):
            ...
            # Check if the current number was included in the optimal solution.
            # This is true if the cardinality at dp[i][j] is greater than at dp[i-1][j].
            if j >= num_value and dp[i - 1][j - num_value] != -1 and \
               dp[i][j] == dp[i-1][j - num_value] + 1:
