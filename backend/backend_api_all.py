...
def batch_query(
    ...
    func_spec: FunctionSpec | None = None,
    **model_kwargs,
) -> list[tuple[OutputType, float, int, int, dict]]:
    """
    Batch query the OpenAI API, optionally with function calling.
    If the model doesn't support function calling, gracefully degrade to text generation.
    """
    _setup_openai_client(api_key=api_key, base_url=base_url)
    filtered_kwargs: dict = select_values(notnone, model_kwargs)
    
    # 确保system_messages和user_messages长度一致
    if system_messages is None:
        system_messages = [None] * len(user_messages)
    if user_messages is None:
        user_messages = [None] * len(system_messages)
    ...
    for i, (system_msg, user_msg) in enumerate(zip(system_messages, user_messages)):
        # 处理Claude模型
        if "claude-" in model_kwargs["model"]:
            if system_msg is not None and user_msg is None:
                system_msg, user_msg = user_msg, system_msg
            if system_msg is not None:
                filtered_kwargs["system"] = system_msg
...
"""
        ...
        results.append((output, req_time, in_tokens, out_tokens, info))
